...
## [阻尼比評分與響應指標分析](damping_rate/damping_rate.md)

> 掃描不同阻尼比，並用回復時間、overshoot、RMS acceleration、peak acceleration 與能量耗散時間建立綜合評分。

此分析用於比較不同阻尼比對單自由度彈簧阻尼系統暫態響應的影響。透過將多個響應指標正規化並加權合成成本分數，可以快速找出目前權重設定下較適合的阻尼比，作為阻尼器初步設定與後續調校方向的參考。
